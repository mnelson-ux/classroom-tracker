...
export default function PinModal({ title, onSubmit, onClose }: Props) {
  ...
  const handleSubmit = async () => {
    if (pin.length !== 5) { setError('Enter your 5-digit student ID'); return }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 backdrop-blur-sm sm:items-center">
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <p className="mb-5 text-center text-sm text-gray-500">Enter your 5-digit student ID</p>

        <input ref={inputRef} type="password" inputMode="numeric" pattern="[0-9]*" maxLength={5}
